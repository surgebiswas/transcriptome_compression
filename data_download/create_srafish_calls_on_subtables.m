function cmd = create_srafish_calls_on_subtables(qt_subtable_direc, organism, varargin)
    ...
    for i = 1 : length(listing)
        ...
        if isempty(donotprocess)
            cmd{i} = sprintf('%s "/proj/dangl_lab/sbiswas/GitHub/transcriptome_compression/data_download/srafish.pl -t %s -i %s -s %s -n %0.0f -o %s -m %0.0f -a %0.0f"', ...
                bsubcall, [td,f], index, opssh, nt, outdir, minreads, maxreads);
        else
            cmd{i} = sprintf('%s "/proj/dangl_lab/sbiswas/GitHub/transcriptome_compression/data_download/srafish.pl -t %s -i %s -s %s -n %0.0f -o %s -m %0.0f -a %0.0f -d %s"', ...
